...
function squircle(t, n) {
   let x = Math.abs(Math.cos(t)) ** (2/n) * Math.sign(Math.cos(t));
   let y = Math.abs(Math.sin(t)) ** (2/n) * Math.sign(Math.sin(t));
   return [x, y];
}
   ...
   gl.useProgram(program);
    ...
    for (let j = 0; j < 16; j++) {
      for (let i = 0; i < num_vertices; i++) {
         let iatio = (j*1000+i)/16000
         let t = i/num_vertices * 2 * Math.PI
         let [x, y] = squircle(t, 4)
         vertices.push([iatio*0.99*x, iatio*0.99*y])
         colours.push(rgba_wheel(t))
      }
      //vertices.push(vertices[j*(num_vertices+1)])
...
function render() {
    ...
    gl.drawArrays(gl.LINE_STRIP, 0, (num_vertices)*16);
