...
{ 
    ...
    if(render_skybox) {
    ...
    } else if (is_final) gl_FragColor = vec4(vec3(0.8), 1.0);
    else {
        // object colour
        ...
        if(gl_FragCoord.x > 850.0/2.0) {
            material_colour = gamma(material_colour, 100);
        }*/
        ...
        if(render_texture) {
            ...
            // reflection only 
            reflection_colour = gamma(reflection_colour, 1.0/2.2);
