...
let projection = false, animate = true, wireframe = false;
let render_texture = true;
let cubemap_prefix = 'texcube_img'
...
async function setup_cubemap(image_prefix)
{
    let images = [];
    for(let k = 0; k < 6; k++) {
        images.push(document.getElementById(image_prefix+k));
        let cube_img = new Image();
        cube_img.crossOrigin = 'anonymous';
        console.log('loaded ' + cube_img.src + images[k].width + ' x ' + images[k].height + ' img');
    }
...
function render_object()
{
    ...
    // D2 MODIFY HERE
    gl.drawElements(wireframe ? gl.LINES : gl.TRIANGLES, mesh.indexBuffer.numItems, gl.UNSIGNED_SHORT, 0);
}
...
async function render() 
{
    ...
    let modelview_skybox = mat_prod(view, model_rotate)
    let view_inv_skybox = mat_prod(view_inv, model_rotate)
    gl.uniformMatrix4fv(projection_loc, false, mat_float_flat_transpose(projection));
    gl.uniformMatrix4fv(modelview_loc, false, mat_float_flat_transpose(modelview_skybox));
    gl.uniformMatrix4fv(view_inv_loc, false, mat_float_flat_transpose(view_inv_skybox));
...
function handle_key_down(event)
{
    ...
    if (event.keyCode == 83) {  // s
        wireframe = false;
        if (cubemap_prefix == 'cubemap_img') {
            cubemap_prefix = 'texcube_img'
            //render_texture = true;
        }
        else  {
            cubemap_prefix = 'cubemap_img';
    ...
    if (event.keyCode == 87) {  // w
        ...
        render_texture = true;
        gl.uniform1i(gl.getUniformLocation(program,'render_texture'), render_texture);
        if (wireframe) setup_cubemap('debug')
        else setup_cubemap(cubemap_prefix)
    }
