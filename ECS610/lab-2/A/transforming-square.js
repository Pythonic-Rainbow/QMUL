...
var translate_loc, shear_loc, projective_loc, projective_inv_loc
...
function render() 
{
    ...
    let pre_scale = mat_identity(4);
    pre_scale[0][0] = 0.5
    pre_scale[1][1] = 0.5

    let pre_rotate = mat_identity(4);
    const xt = Math.PI / 4
    pre_rotate[0] = [Math.cos(xt), -Math.sin(xt), 0, 0]
    pre_rotate[1] = [Math.sin(xt), Math.cos(xt), 0, 0]
    pre_rotate[2] = [0, 0, 1, 0]
    pre_rotate[3] = [0, 0, 0, 1]
