...
translation = [[1, 0,  0, max_depth/2],
               [0, 1, 0, max_depth/2],
               [0, 0, 1, 0],
               [0, 0, 0, 1]];

// control flags
var use_colour_loc, alpha_loc, custom_colour_loc;
...
window.onload = async function()
{
    ...
    custom_colour_loc = gl.getUniformLocation(program, 'custom_colour')
...
function render_control()
{
    ...
    theta += theta_step * 5;
    var camera_angle = theta_step * 2;
    let x =eye[0], z=eye[2]
    eye[0] = x * Math.cos(camera_angle) + z * Math.sin(camera_angle);
    eye[2] = -x * Math.sin(camera_angle) + z * Math.cos(camera_angle);
...
function render() 
{
    ...
    gl.uniform1i(custom_colour_loc, true)
    ...
    const identity = mat_float_flat_transpose(mat_identity(4))
    gl.uniformMatrix4fv(translation_loc, false, identity);
    gl.uniformMatrix4fv(rotation_loc, false, identity);
    gl.uniform1i(custom_colour_loc, false)
